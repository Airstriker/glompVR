...
uniform int textureEnvModeCombine; // texture environment mode uniform: Replace = 7681, Modulate = 8448 (check OpenTK enum (named the same)),
in vec2 ftexcoord;
in vec4 vWorldSpacePos;  // position of the vertex (and fragment) in world space
//in vec4 vEyeSpacePos;  // position of the vertex (and fragment) in eye space
...
lightSource light0 = lightSource(
	vec4(-50.0, 100.0,  20.0, 1.0), //Point light
...
void main() {
	...
	if (textureEnvModeCombine == 7681) //Replace texture mode - for ice cube effect
	{
		FragColor = texture(tex, ftexcoord); //No lightening
	}
	else if (textureEnvModeCombine == 8448) //Modulate texture mode
	{
		lights[0] = light0;
